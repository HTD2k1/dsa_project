...
function GameTable(props) {
  ...
  const styleContainer = {
    ...
    invalid:  {backgroundColor: "hsl(7, 63%, 67%)", color: "hsl(210, 88%, 56%)"},
    unchangeable: {color: 'black'},
    unchangeablyInvalid: {backgroundColor: "hsl(7, 63%, 67%)", color: 'black'}
  }
              ...
              return (
                <tr key={100000 + suRowIndex}>
                  {suRow.map((suValue, suColIndex) => {
                    ...
                    // Style condition
                    const isInvalid = invalidSet &&  invalidSet.has(suRowIndex+","+suColIndex)

                    //Styling input cell
                    let normalCell = (isInvalid) ? 
                    styleContainer.invalid : null;
                     normalCell = (isCellSelected(suRowIndex,suColIndex)) ? styleContainer.selected : normalCell;
                    //Styling unchangeable cell
                    const unchangeableCell = (isInvalid) ? 
                    styleContainer.unchangeablyInvalid : styleContainer.unchangeable;  

                    if(unchangeableSet && !unchangeableSet.has(suRowIndex+","+suColIndex)) // checking if <td> is changeable
                    return (
